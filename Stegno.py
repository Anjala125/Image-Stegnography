...
'''
    ...
    def main(self, root):
        ...
        title = Label(f, text='Image Steganography', bg='#2c3e50', fg='#ecf0f1')
        ...
        if not myfile:
            ...
            if password:
                try:
                    ...
                    text_area.grid()
                    languages = {'Spanish': 'es', 'French': 'fr', 'German': 'de', 'Chinese': 'zh-cn','English': 'en', 'Hindi': 'hi', 'Arabic': 'ar', 'Russian': 'ru', 'Japanese': 'ja', 'Korean': 'ko'}
                    selected_lang = StringVar(d_f3)
                    selected_lang.set('Spanish')  # default value
    
                    l3 = Label(d_f3, text='Select language for translation:', bg='#2c3e50', fg='#ecf0f1')
                    l3.config(font=('Helvetica', 18))
                    l3.grid(pady=10)
                    lang_menu = OptionMenu(d_f3, selected_lang, *languages.keys())
                    lang_menu.config(bg='#3498db', fg='#ecf0f1', font=('Helvetica', 14, 'bold'))
                    lang_menu.grid()
    
                    def translate_text():
                        translator = Translator(to_lang=languages[selected_lang.get()])
                        translated = translator.translate(decrypted_data)
                        l4 = Label(d_f3, text='Translated data is:', bg='#2c3e50', fg='#ecf0f1')
                        l4.config(font=('Helvetica', 18))
                        l4.grid(pady=10)
                        translated_text_area = Text(d_f3, width=50, height=10, bg='#34495e', fg='#ecf0f1')
                        translated_text_area.insert(INSERT, translated)
                        translated_text_area.configure(state='disabled')
                        translated_text_area.grid()
                        self.read_aloud(translated)
    
                    translate_button = Button(d_f3, text='Translate', command=translate_text, bg='#3498db', fg='#ecf0f1')
                    translate_button.config(font=('Helvetica', 14, 'bold'))
                    translate_button.grid(pady=10)
                    voice_button = Button(d_f3, text='Read Aloud', command=lambda: self.read_aloud(decrypted_data), bg='#3498db', fg='#ecf0f1')
                    voice_button.config(font=('Helvetica', 14, 'bold'))
